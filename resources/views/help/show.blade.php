...
@section('content')
    <form method="post">
        ...
        @if( !empty($user))
            ...
            <input type="submit" id="submit" value="Помочь"{{ !empty($user)?:' disabled="disabled"' }}><br>
            <span class="description"><span class="required">*</span> - Обязательное к заполнению поле</span>
        @else
            <p>Нам необходимо знать хотя бы один из ваших аккаунтов, чтобы у нас была возможность связаться с вами.</p>
        @endif
    </form>
